...
const { requiresChannelAccess, requiresAuth } = require('../helpers/permissions');
...
module.exports = {
  ...
  Mutation: {
    ...
    findMessages: requiresAuth.createResolver(
      async (parent, { text }, { models, user }) =>
        await models.Message.find({ text: { $regex: text }, userId: user.id })
    ),
  },
  Message: {
